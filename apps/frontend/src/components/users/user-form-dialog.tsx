  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-3">
        <p className="text-xs font-semibold uppercase tracking-widest text-muted-foreground whitespace-nowrap">{title}</p>
...
export function UserFormDialog({ open, onClose, onSuccess, editData }: Props) {
  ...
  useEffect(() => {
    ...
    api.get('/master/balai').then((b) => setBalaiList(b.data)).catch(() => {});
      ...
      >
        <DialogHeader className="px-6 pt-6 pb-4 border-b shrink-0">
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            {isEdit ? 'Perbarui informasi pengguna' : 'Isi informasi pengguna baru'}
          </p>
        </DialogHeader>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <Section title="Informasi Akun">
            ...
            <div className="space-y-1.5">
              <Label>{isEdit ? 'Password Baru' : 'Password'} {!isEdit && <span className="text-destructive">*</span>}</Label>
              <Input
                type="password"
          ...
          <Section title="Profil">
            <div className="space-y-1.5">
              ...
              {errors.name && <p className="text-destructive text-xs">{String(errors.name.message)}</p>}
            </div>
            ...
            <div className="space-y-1.5">
              ...
              {errors.email && <p className="text-destructive text-xs">{String(errors.email.message)}</p>}
            </div>
          </Section>
          ...
          <Section title="Akses & Unit Kerja">
            <div className="space-y-1.5">
              ...
              {errors.roleCode && <p className="text-destructive text-xs">{String(errors.roleCode.message)}</p>}
            </div>
                ...
                <SelectContent>
                  {balaiList.map((b) => (
                    <SelectItem key={b.id} value={b.id.toString()}>
                      ...
                      <span className="text-muted-foreground ml-2">— {b.name}</span>
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
